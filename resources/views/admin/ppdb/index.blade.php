							...
							<tbody>
								...
								@foreach ($ppdb as $item)
									<tr>
										...
										<td>{{ $item->email }}</td>
										<td>
											<img src="{{ asset('storage/' . $item->pas_foto) }}" alt="Foto {{ $item->nama }}"
												class="img-thumbnail w-50">
										</td>
										<td>{{ $item->jurusan }}</td>
										<td>
											<span
												class="badge @if ($item->status == 'menunggu') bg-primary
                                    @elseif ($item->status == 'diterima')
                                        bg-success
                                    @elseif ($item->status == 'ditolak')
                                        bg-danger @endif">{{ $item->status }}</span>
										</td>
										<td>
											<button class="btn btn-info btn-sm" data-bs-toggle="modal" data-bs-target="#modalDetail{{ $item->id }}"><i
													class="bx bx-info-circle"></i> Detail</button>
											<button class="btn btn-warning btn-sm" data-bs-toggle="modal"
												data-bs-target="#modalStatus{{ $item->id }}"><i class="bx bx-pen"></i> Status</button>
											<button class="btn btn-danger btn-sm" data-bs-toggle="modal"
												data-bs-target="#modalDelete{{ $item->id }}"><i class="bx bx-trash"></i> Hapus</button>
										</td>
									</tr>
								@endforeach
							</tbody>
			...
			<div class="modal-dialog modal-xl modal-dialog-centered">
				<div class="modal-content">
					<div class="modal-header bg-info pb-3">
						<h5 class="modal-title text-white" id="modalDetailTitle">Detail Pendaftar</h5>
						<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
					</div>
					<div class="modal-body">
						<div class="row">
							<div class="col-12 mb-3">
								<img class="img-thumbnail mx-auto shadow" width="200" src="{{ asset('storage/' . $item->pas_foto) }}"
								...
								<div class="table-responsive text-nowrap">
									<table id="table" class="table table-hover">
										<tbody>
											...
											<tr>
												<td class="bg-info text-white">Scan Raport</td>
												<td>
													@if ($item->scan_raport)
														<img class="img-thumbnail mx-auto shadow" width="200"
															src="{{ asset('storage/' . $item->scan_raport) }}" alt="Scan Raport">
														<a href="{{ asset('storage/' . $item->scan_raport) }}" target="_blank"
											...
											<tr>
												<td class="bg-info text-white">Scan Akta</td>
												<td>
													@if ($item->scan_akta)
														<img class="img-thumbnail mx-auto shadow" width="200"
															src="{{ asset('storage/' . $item->scan_akta) }}" alt="Scan Akta">
														<a href="{{ asset('storage/' . $item->scan_akta) }}" target="_blank"
															class="btn btn-sm btn-primary mt-2">View File</a>
													@else
														<span class="badge bg-secondary">Not Available</span>
													@endif
											...
											<tr>
												<td class="bg-info text-white">Scan KK</td>
												<td>
													@if ($item->scan_kk)
														<img class="img-thumbnail mx-auto shadow" width="200" src="{{ asset('storage/' . $item->scan_kk) }}"
															alt="Scan KK">
														<a href="{{ asset('storage/' . $item->scan_kk) }}" target="_blank"
															class="btn btn-sm btn-primary mt-2">View File</a>
													@else
														<span class="badge bg-secondary">Not Available</span>
													@endif
											...
											<tr>
												<td class="bg-info text-white">Scan Piagam 1</td>
												<td>
													@if ($item->scan_piagam1)
														<img class="img-thumbnail mx-auto shadow" width="200"
															src="{{ asset('storage/' . $item->scan_piagam1) }}" alt="Scan Piagam 1">
														<a href="{{ asset('storage/' . $item->scan_piagam1) }}" target="_blank"
															class="btn btn-sm btn-primary mt-2">View File</a>
													@else
														<span class="badge bg-secondary">Not Available</span>
													@endif
											...
											<tr>
												<td class="bg-info text-white">Scan Piagam 2</td>
												<td>
													@if ($item->scan_piagam2)
														<img class="img-thumbnail mx-auto shadow" width="200"
															src="{{ asset('storage/' . $item->scan_piagam2) }}" alt="Scan Piagam 2">
														<a href="{{ asset('storage/' . $item->scan_piagam2) }}" target="_blank"
															class="btn btn-sm btn-primary mt-2">View File</a>
													@else
														<span class="badge bg-secondary">Not Available</span>
													@endif
											...
											<tr>
												<td class="bg-info text-white">Scan Piagam 3</td>
												<td>
													@if ($item->scan_piagam3)
														<img class="img-thumbnail mx-auto shadow" width="200"
															src="{{ asset('storage/' . $item->scan_piagam3) }}" alt="Scan Piagam 3">
														<a href="{{ asset('storage/' . $item->scan_piagam3) }}" target="_blank"
															class="btn btn-sm btn-primary mt-2">View File</a>
													@else
														<span class="badge bg-secondary">Not Available</span>
													@endif
